...
params: 
  ...
  in_preset_IC_Fuel_Economy: in_preset_IC_Fuel_Economy
  flag_in_preset_IC_Fuel_Economy: flag_in_preset_IC_Fuel_Economy
...
#Pre allocate the table
N = 2
IC <- data.frame(Name = rep("", N), Value = rep(NA, N), Unit = rep("", N), Preset = rep("", N), stringsAsFactors = F)

#Learn if a preset was used or not
if (params$flag_in_preset_IC_Fuel_Economy) {
  IC_preset = params$in_preset_IC_Fuel_Economy
} else {IC_preset = "User Defined"}

#Build the table
IC[1,] <- list("Average Fuel Economy", params$IC_Fuel_Economy, "mpg", IC_preset)
IC[2,] <- list("Carbon Emissions from Gasoline", drop_units(set_units(set_units(1, "gal"), CO2_g)), "g CO2/gal", "(Constant)")
...
The IC (internal combustion engine) table shows the average fuel economy of an automobile. The carbon emissions from gasoline was obtained from the [US EPA](https://www.epa.gov/energy/greenhouse-gases-equivalencies-calculator-calculations-and-references).
...
if (params$EBike_include) { #only include if "include" tickbox checked in GUI
  ...
  } else {EBike_preset = "User Defined"}
  
  #Build the table
  EBike[1,] <- list("Average Range", params$EBike_Range, "mi", EBike_preset)
  EBike[2,] <- list("Average Battery Storage", params$EBike_Battery_Storage, "kWh", EBike_preset)
  EBike[3,] <- list("Ratio, portion of car miles replaced by e-bike (maximum 1)", params$EBike_VMT_r, "", "User Defined")
...
`r if (params$EBike_include) {"The E-Bike table shows the average range and battery storage of e-bikes eligible for an e-bike incentive.These values can be obtained from e-bike manufacturers. Additionally, the ratio of automobile miles that are replaced by e-bike miles is specified. This value can be obtained from local e-bike use studies."}`
...
if (params$PHEV_include) { #only include if "include" tickbox checked in GUI
  ...
  PHEV[3,] <- list("Average IC Mode Fuel Economy", params$PHEV_ic_econ, "mpg", PHEV_preset)
